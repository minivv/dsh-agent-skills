...
export function builtinAgentSkillDirs(home = homedir(), agentsHome = process.env.DSH_AGENTS_HOME) {
    return AGENT_SKILL_DIRS.map(([, rel]) => rel === join(".agents", "skills")
        ? join(agentsHome ?? join(home, ".agents"), "skills")
        : join(home, rel)).map((path) => resolve(path));
}
