...
function isPresetPackageRoot(dir) {
    ...
    try {
        const pkg = JSON.parse(readFileSync(manifest, "utf8"));
        if (pkg.name !== DSH_PRESET_PACKAGE)
            return false;
...
function verifiedPresetPackageRoot(start) {
...
export function resolveDshPackageRoot(options = {}) {
    ...
    for (const candidate of candidates) {
        ...
        if (candidate !== undefined && candidate !== "") {
            if (isPresetPackageRoot(candidate) || isLegacyDshRoot(candidate))
            ...
            try {
                ...
                if (isPresetPackageRoot(dir) || isLegacyDshRoot(dir))
...
function presetFiles(root) {
    ...
    for (const presetRoot of candidates) {
        for (const id of PRESET_IDS) {
            ...
            if (existsSync(file) && !files.includes(file))
                files.push(file);
        }
    }
    if (files.length === 0)
    ...
    return PRESET_IDS.map((id) => files.find((f) => f.endsWith(`/${id}/agent.cordis.yml`))).filter((f) => f !== undefined);
}
